"""Prepare ASGI application for landing."""

from collections.abc import Callable
from collections.abc import Sequence
from pathlib import Path
from typing import Any

from litestar import Litestar
from litestar.config.cors import CORSConfig
from litestar.contrib.jinja import JinjaTemplateEngine
from litestar.exceptions import HTTPException
from litestar.static_files import create_static_files_router
from litestar.template import TemplateConfig
from litestar.types import ControllerRouterHandler

from src.infra.litestar.builder import LitestarBuilder
from src.infra.litestar.exception_handlers.landing import internal_server_exception_handler
from src.infra.litestar.exception_handlers.landing import not_found_exception_handler
from src.infra.plugins.configs import compression
from src.infra.plugins.configs import cors
from src.presentation.landing import landing_router


class LandingLitestarBuilder(LitestarBuilder):
    """Litestar application builder."""

    @staticmethod
    def get_route_handlers() -> Sequence[ControllerRouterHandler]:
        """Get route handlers."""
        return [
            *landing_router,
            create_static_files_router(
                path="/static",
                directories=["static/landing"],
                name="static",
            ),
            create_static_files_router(
                path="/media",
                directories=["."],
                name="media"
            ),
        ]

    @staticmethod
    def get_cors_config() -> CORSConfig | None:
        """Get CORS config."""
        return cors

    @staticmethod
    def get_template_config() -> TemplateConfig | None:
        """Get template config."""
        return TemplateConfig(
            directory=Path("templates"),
            engine=JinjaTemplateEngine,
        )

    @staticmethod
    def get_exception_handlers() -> (
        dict[type[Exception], Callable]
    ):
        """Get exception handlers."""
        return {
            HTTPException: not_found_exception_handler,
            Exception: internal_server_exception_handler,
        }

    @staticmethod
    def get_compression_config() -> Any | None:
        """Get compression config."""
        return compression


builder: LandingLitestarBuilder = LandingLitestarBuilder()
app: Litestar = builder.build()
