...
def create_roles() -> None:
    ...
    #         default_role = await roles_service.
    #         get_one_or_none(slug=slugify(users_service.default_role))
    #         if default_role:
...
#     statement=select(Role).options(load_only(
#     Role.id, Role.slug, Role.name,
#     Role.description)),
#     config=alchemy,
# ) as service:
#     fixture_data = await open_fixture_async(
#     fixtures_path, "role")
...
#         async with alchemy.get_session() as db_session:
#             users_service = await
#             anext(provide_users_service(db_session))
#             user = await users_service.create(data=obj
#             _in.to_dict(), auto_commit=True)
...
#     superuser = superuser or click.prompt("Creat
#     e as superuser?", show_default=True, type=click.BOOL)
#
#     anyio.run(_create_user, email, name, password, superuser)
#
#
# @user_management_app.command(name="promote-to-superuser", hel
# p="Promotes a user to application superuser")
# @click.option(
#     "--email",
